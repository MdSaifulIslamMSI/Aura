...
- The PQC Semgrep lane (`security/semgrep/pqc-crypto-policy.yml`) mirrors the expiring exceptions in `config/security/pqc-allowlist.json`: the five hash sites (`server/services/productImageResolver.js`, `server/services/catalogArtworkService.js`, `scripts/i18n/codemod-jsx-stable-text.mjs`, `scripts/i18n/discover-stable-ui-text.mjs`, `server/services/listingService.js`) are excluded at the rule level via `paths.exclude` — the operative mechanism. Each file also carries a `// nosemgrep: security.semgrep.nodejs-sha1|nodejs-md5` comment (fully qualified: semgrep namespaces local-config rules by their config path) documenting the acceptance in code. Each hash is a deterministic identifier/correlation value, not a security primitive; the node-side allowlist entries (owner: maintainer, expires 2026-12-31) remain the canonical record. Deliberate bad-example fixtures under `tests/fixtures/security/pqc/`, the policy surfaces themselves, and `server/tests/` scanner-behavior assertions are excluded from the relevant rules because they are the scanner's own test corpus.
