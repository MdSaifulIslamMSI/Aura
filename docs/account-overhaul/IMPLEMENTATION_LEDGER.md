...
| M | Performance and scalability | Static bundle implementation complete; authenticated runtime measurements pending | `account: complete accessibility and performance hardening` | Every secondary account section is route-state lazy loaded behind a stable-dimension fallback and keyed section error boundary. Frontend bundle budget PASS: 175.97 kB initial JS gzip, 65.88 kB total CSS, 60.44 kB initial CSS and 243.82 kB initial payload; Profile fell to 16.91 kB gzip from the Wave I 27.04 kB observation, and every account feature chunk is at or below 11.82 kB gzip versus the 45 kB feature cap. Authenticated request counts, Core Web Vitals, API latency and constrained-device traces remain staging gates. |
| N | Observability and analytics | Code complete on integration branch; staging activation/calibration pending | `account: complete observability and analytics` | Typed allowlisted product events and native LCP/INP/CLS collection feed privacy-safe server normalization. Server-authoritative operation outcome/latency, privacy-job, client diagnostic and migration metrics use bounded labels. Raw session/IP values are HMAC-referenced; URLs lose queries/dynamic IDs; user agents become coarse families. A 10-panel Grafana dashboard and 8 Prometheus alerts are provisioned by existing wildcard contracts. Backend: 7 focused suites / 23 tests across telemetry, privacy, errors and migration PASS; frontend: 5 files / 13 tests PASS; route scanners, 2,665-file secret scan, lint, observability asset validation, localization checks and bundle budget PASS. Real scrape, dashboard, alert-routing, cardinality, retention and SLO calibration remain staging gates. |
