...
| Custom PQC deployment proof | Local Node.js script | Controllable-surface evidence for SSH, TLS edge, OpenSSL/OQS lab, internal service encryption, and backup key-agility | `npm run security:pqc:proof` | Runs in `post-quantum-security.yml` and uploads proof artifacts. |
