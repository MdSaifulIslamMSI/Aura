...
- If the keystore secrets are missing, CI still produces a GitHub APK signed with Aura's repo public internal update key. This keeps GitHub APK updates installable across releases without requiring you to own a signing certificate, but it is not production store trust.
