...
The release workflow can build unsigned artifacts immediately. For a production experience like major desktop apps, add signing and notarization before public rollout:

- Windows: add an Authenticode code-signing certificate and enable executable signing.
...
Without platform signing, users may see operating-system trust warnings even though the files are generated by CI.
