...
const main = () => {
    const result = scanSensitiveRoutes();
    if (result.exceptionErrors.length) {
        console.error('Sensitive route exception errors:');
        for (const error of result.exceptionErrors) console.error(`- ${error}`);
    }
    if (result.findings.length) {
        console.error('Sensitive route coverage findings:');
        for (const finding of result.findings) {
            console.error(`- ${finding.key} (${finding.reason})`);
        }
    }
    if (!result.ok) {
        process.exitCode = 1;
    ...
    console.log(`Sensitive route coverage passed for ${path.relative(repoRoot, result.routeDir).replace(/\\/g, '/')}`);
};
