...
// Cloudflare Pages serves the uploaded artifact as-is; the generated
// `cloudflare/_headers` file carries the same security-header contract as
// every other lane (the CSP is checked by scripts/security/check-csp-drift.mjs
// as the seventh copy). Cache tiers mirror the Vercel/Caddy lanes, but each
// block sets disjoint header names (Cache-Control only on asset/document
// blocks) so the result does not depend on Cloudflare's multi-block merge
// precedence. `cloudflare/_redirects` pins the SPA fallback explicitly
// instead of relying on Pages' undocumented default routing, so deep links
// (react-router routes) survive refresh on this lane.
// Regenerated by app/scripts/sync_vercel_configs.mjs.
export const buildCloudflarePagesHeaders = (origin = HOSTED_BACKEND_ORIGIN) => {
    assertAbsoluteHttpUrl(origin);
    ...
    return [
        ...
        renderBlock('/index.html', [{ key: 'Cache-Control', value: FRONTEND_DOCUMENT_CACHE_CONTROL }]),
    ].join('\n');
};

export const buildCloudflarePagesRedirects = () => '/* /index.html 200\n';
