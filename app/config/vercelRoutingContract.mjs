...
export const FRONTEND_SECURITY_HEADERS = buildFrontendSecurityHeaderValues(HOSTED_BACKEND_ORIGIN);
 ...
 // Gateway launch surface policy. Deliberately separate from the storefront
 // policy: the gateway reads release metadata from api.github.com and never
 // talks to the backend edge, so it carries no backend connect-src entries.
 // (Kept out of the CSP drift gate, which compares the storefront copies.)
 export const buildGatewayContentSecurityPolicy = () => [
     "default-src 'self'",
     "base-uri 'self'",
     "object-src 'none'",
...
export const buildRenderGatewayServiceLines = () => {
    const headers = [
        { key: 'Content-Security-Policy', value: buildGatewayContentSecurityPolicy() },
    ...
    return [
        ...
        '    name: aura-gateway',
        '    runtime: static',
        '    buildCommand: echo "gateway is fully static; nothing to build"',
        '    staticPublishPath: gateway',
        '    previews:',
