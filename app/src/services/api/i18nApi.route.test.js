// AUTO-GENERATED route-contract suite for i18nApi.js — do not edit by hand.
// Regenerate with: node scripts/generate-api-client-tests.cjs i18nApi.js
...
describe('i18nApi route contract', () => {
    // Override: Method de-duplicates `texts` and returns early (no apiFetch) when the list is empty, so placeholder args never reach the network.
    it("translateTexts calls POST /i18n/translate", async () => {
        await i18nApi.translateTexts({ texts: ['hello'], language: 'fr' }).catch(() => {});
        expect(apiFetch).toHaveBeenCalled();
        const [reqPath, options = {}] = apiFetch.mock.calls[apiFetch.mock.calls.length - 1];
        expect(String(options.method || 'GET')).toBe('POST');
        expect(String(reqPath)).toBe("/i18n/translate");
    });
