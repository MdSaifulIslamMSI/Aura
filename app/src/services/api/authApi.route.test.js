// AUTO-GENERATED route-contract suite for authApi.js — do not edit by hand.
// Regenerate with: node scripts/generate-api-client-tests.cjs authApi.js
...
import { authApi, otpApi } from './authApi';
...
describe('authApi route contract', () => {
    it("getSession calls GET /auth/session", async () => {
        await authApi.getSession(anyArgs).catch(() => {});
        expect(apiFetch).toHaveBeenCalled();
        const [reqPath, options = {}] = apiFetch.mock.calls[apiFetch.mock.calls.length - 1];
        expect(String(options.method || 'GET')).toBe('GET');
        expect(String(reqPath)).toBe("/auth/session");
        expect(options.headers && options.headers.Authorization).toBe('Bearer test');
    });

    it("logoutSession calls POST /auth/logout", async () => {
        await authApi.logoutSession(anyArgs).catch(() => {});
        expect(apiFetch).toHaveBeenCalled();
        const [reqPath, options = {}] = apiFetch.mock.calls[apiFetch.mock.calls.length - 1];
        expect(String(options.method || 'GET')).toBe('POST');
        expect(String(reqPath)).toBe("/auth/logout");
        expect(options.headers && options.headers.Authorization).toBe('Bearer test');
    });

});

describe('otpApi route contract', () => {
    // Override: First source call is the env-conditional PoW /otp/challenge probe; in NODE_ENV=test sendOtp posts directly to the first candidate path.
    it("sendOtp calls POST /auth/otp/send", async () => {
        await otpApi.sendOtp(anyArgs).catch(() => {});
        expect(apiFetch).toHaveBeenCalled();
        const [reqPath, options = {}] = apiFetch.mock.calls[apiFetch.mock.calls.length - 1];
        expect(String(options.method || 'GET')).toBe('POST');
        expect(String(reqPath)).toBe("/auth/otp/send");
    });
