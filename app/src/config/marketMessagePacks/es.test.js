...
describe('marketMessagePacks/es', () => {
  ...
  it('localizes known UI keys rather than echoing en', () => {
    const uiKeys = ['checkout.continue', 'market.title', 'nav.preferences'];
    uiKeys.forEach((key) => expect(PACK[key]).toBeTypeOf('string'));
    expect(uiKeys.some((key) => PACK[key] !== EN_PACK[key])).toBe(true);
  });
});
