        ...
        <div className="space-y-5">
            <div className="premium-panel premium-card-hover p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-6">
                    <h2 className="text-xl font-black text-white">{t('profile.orders.title', {}, 'Order History')}</h2>
                    <span className="text-sm text-slate-400">{t('profile.orders.total', { count: formatNumber(stats.totalOrders || 0) }, `${formatNumber(stats.totalOrders || 0)} total orders`)}</span>
                </div>
                {recentOrders.length === 0 ? (
                    <EmptyState
                        icon={Package}
                        title={t('profile.orders.emptyTitle', {}, 'No orders yet')}
                        body={t('profile.orders.emptyBody', {}, 'Start shopping to see your orders here')}
                        action={(
                            <Link to="/products" className="inline-flex min-h-11 items-center px-6 py-2 rounded-xl bg-[#d2a96c] font-bold text-sm text-[#17231e] transition-colors hover:bg-[#f3c982]">
                                {t('profile.orders.shopNow', {}, 'Shop Now')}
                            </Link>
                        )}
                    />
                ) : (
                        ...
                        {recentOrders.map(order => (
                            <div key={order._id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 transition-colors hover:bg-white/[0.05]">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                                    <div>
                                        <p className="text-xs text-slate-400">{t('profile.orders.orderNumber', { id: order._id?.slice(-8).toUpperCase() }, `Order #${order._id?.slice(-8).toUpperCase()}`)}</p>
                                        <p className="text-xs text-slate-400">{formatDateTime(order.createdAt, undefined, { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                                    </div>
                ...
                ${order.isDelivered ? 'border-emerald-400/20 bg-emerald-500/12 text-emerald-200' : order.isPaid ? 'border-blue-400/20 bg-blue-500/12 text-blue-200' : 'border-amber-400/20 bg-amber-500/12 text-amber-200'}`}>
                                            {order.isDelivered
                                                ? t('profile.orders.status.delivered', {}, 'Delivered')
                                                : order.isPaid
                                                    ? t('profile.orders.status.shipped', {}, 'Shipped')
                                                    : t('profile.orders.status.processing', {}, 'Processing')}
                                        </span>
                                    </div>
                                </div>
                        ...
                        <Link to="/orders" className="block py-3 text-center text-sm font-bold text-neo-cyan hover:underline">
                            {t('profile.orders.viewAll', {}, 'View All Orders →')}
                        </Link>
                    </div>
                )}
            </div>
