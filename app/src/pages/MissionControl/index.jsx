        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <form onSubmit={runMission} className="rounded-3xl border border-white/10 bg-white/[0.045] p-6 shadow-glass">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <input type="text" value={form.hints} onChange={(event) => updateForm('hints', event.target.value)} placeholder={t('common.jsx.expression.screenshot.hint.owned.device.favorite.brand.c00d0b60', {}, 'Screenshot hint, owned device, favorite brand')} className="md:col-span-2 h-12 rounded-2xl border border-white/10 bg-zinc-950/70 px-4 text-sm text-white outline-none placeholder:text-slate-500 focus:border-cyan-300/35" />
              <input type="url" value={form.imageUrl} onChange={(event) => updateForm('imageUrl', event.target.value)} placeholder={t('search.jsx.expression.optional.image.url.for.visual.search.3b28096f', {}, 'Optional image URL for visual search')} className="md:col-span-2 h-12 rounded-2xl border border-white/10 bg-zinc-950/70 px-4 text-sm text-white outline-none placeholder:text-slate-500 focus:border-cyan-300/35" />
            </div>
