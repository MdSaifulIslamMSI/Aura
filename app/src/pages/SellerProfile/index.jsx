...
export default function SellerProfile() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await listingApi.getSellerProfile(id);
        setData(result);
      } catch (error) {
        console.error(error);
      } finally {
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[#04060f]">
        <div className="h-12 w-12 animate-spin rounded-full border-4 border-cyan-300/70 border-t-transparent" />
        <p className="text-sm text-slate-300">{t('sellerProfile.loading', {}, 'Loading seller profile...')}</p>
      </div>
    );
  }
  ...
  if (!data?.seller) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#04060f] px-4 text-center text-slate-100">
        <div>
          <h2 className="mb-2 text-2xl font-black">{t('sellerProfile.notFoundTitle', {}, 'Seller not found')}</h2>
          <Link
            to="/marketplace"
          ...
          >
            ...
            {t('sellerProfile.backToMarketplace', {}, 'Back to Marketplace')}
          </Link>
        </div>
      </div>
    );
  ...
  const memberSince = formatDateTime(seller.createdAt, undefined, { month: 'long', year: 'numeric' });
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
            ...
            <div>
              ...
              <p className="mt-1 flex items-center gap-2 text-sm text-cyan-100/70">
                ...
                {t('sellerProfile.memberSince', { date: memberSince }, `Member since ${memberSince}`)}
              </p>
              <div className="mt-4 grid grid-cols-2 gap-3 sm:flex sm:gap-4">
                <div className="rounded-xl border border-cyan-300/25 bg-slate-900/45 px-4 py-2 text-center">
                  <p className="text-2xl font-black">{formatNumber(seller.activeListings)}</p>
                  <p className="text-xs text-cyan-100/65">{t('sellerProfile.stats.activeListings', {}, 'Active listings')}</p>
                </div>
                <div className="rounded-xl border border-cyan-300/25 bg-slate-900/45 px-4 py-2 text-center">
                  <p className="text-2xl font-black">{formatNumber(seller.totalSold)}</p>
                  <p className="text-xs text-cyan-100/65">{t('sellerProfile.stats.soldItems', {}, 'Sold items')}</p>
                </div>
                {passport && (
                  <div className="rounded-xl border border-emerald-300/25 bg-emerald-500/10 px-4 py-2 text-center">
                    <p className="text-2xl font-black text-emerald-100">{formatNumber(passport.trustScore)}</p>
                    <p className="text-xs text-emerald-100/70">{t('sellerProfile.stats.trustScore', {}, 'Trust score')}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        {passport && (
          <div className="mb-8 rounded-3xl border border-cyan-300/20 bg-slate-900/70 p-6">
            <h2 className="mb-4 text-lg font-black">{t('sellerProfile.passportTitle', {}, 'Seller Trust Passport')}</h2>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-xl border border-white/10 bg-slate-950/40 px-4 py-3">
                <p className="text-xs uppercase tracking-wider text-slate-400">{t('sellerProfile.passport.fraudRisk', {}, 'Fraud Risk')}</p>
                <p className="text-lg font-black uppercase">{passport.fraudRiskTier}</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-slate-950/40 px-4 py-3">
                <p className="text-xs uppercase tracking-wider text-slate-400">{t('sellerProfile.passport.disputeRate', {}, 'Dispute Rate')}</p>
                <p className="text-lg font-black">{formatNumber(passport.disputeRate)}%</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-slate-950/40 px-4 py-3">
                <p className="text-xs uppercase tracking-wider text-slate-400">{t('sellerProfile.passport.onTimeHistory', {}, 'On-time History')}</p>
                <p className="text-lg font-black">{formatNumber(passport.onTimeHistory)}%</p>
              </div>
              <div className="rounded-xl border border-white/10 bg-slate-950/40 px-4 py-3">
                <p className="text-xs uppercase tracking-wider text-slate-400">{t('sellerProfile.passport.responseSla', {}, 'Response SLA')}</p>
                <p className="text-lg font-black">{formatNumber(passport.responseSlaHours)}h</p>
              </div>
            </div>
            ...
            {passport.fraudRiskTier === 'high' && (
              <p className="mt-4 inline-flex items-center gap-1.5 rounded-lg border border-amber-300/35 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-100">
                ...
                {t('sellerProfile.passport.highRiskNotice', {}, 'High risk tier means extra care is recommended before payment.')}
              </p>
            )}
          </div>
        )}
        ...
        <h2 className="mb-6 text-xl font-black">
          {t('sellerProfile.activeListingsTitle', { count: formatNumber(listings.length) }, `Active Listings (${formatNumber(listings.length)})`)}
        </h2>
        ...
        {listings.length === 0 ? (
          <div className="rounded-3xl border border-slate-800 bg-slate-900/60 py-16 text-center">
            ...
            <p className="text-slate-400">{t('sellerProfile.emptyListings', {}, 'This seller has no active listings right now.')}</p>
          </div>
        ) : (
                ...
                <div className="relative aspect-[4/3] overflow-hidden bg-slate-800/70">
                  ...
                  {listing.negotiable && (
                    <span className="absolute left-2 top-2 rounded-full bg-emerald-500/90 px-2 py-0.5 text-[10px] font-black tracking-wide text-white">
                      {t('sellerProfile.negotiable', {}, 'Negotiable')}
                    </span>
                  )}
                </div>
                <div className="p-3">
                  <p className="text-lg font-black text-slate-100">{formatPrice(listing.price, 'INR')}</p>
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    {t(
                      ...
                      { time: formatRelativeSellerTime(listing.createdAt, t, formatNumber) },
                      `Posted ${formatRelativeSellerTime(listing.createdAt, t, formatNumber)}`
                    )}
                  </div>
                </div>
              </Link>
