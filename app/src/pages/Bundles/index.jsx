...
export default function Bundles() {
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                const response = await aiApi.chat({
                    message: t('bundles.ai.prompt.explainGroundedBundle', {}, 'Explain this grounded bundle for the shopper, highlight tradeoffs, and suggest next actions.'),
    ...
    }, [budget, bundle, maxItems, t, theme]);
        ...
        <div className="min-h-screen pb-16">
            <div className="container-custom max-w-7xl mx-auto px-4 py-8">
                <div className="rounded-3xl border border-cyan-300/25 bg-gradient-to-r from-cyan-500/12 via-violet-500/10 to-emerald-500/12 p-6 md:p-8">
                    <p className="text-[11px] font-black uppercase tracking-[0.2em] text-cyan-100"><StableText id={"common.jsx.text.smart.bundle.builder.196a2b01"} defaultMessage={"Smart Bundle Builder"} /></p>
                ...
                <div className="mt-6 grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6">
                    <aside className="rounded-2xl border border-white/10 bg-white/5 p-5 h-fit">
                        <h2 className="text-sm font-black uppercase tracking-[0.16em] text-white mb-4 flex items-center gap-2">
                            ...
                            <StableText id={"common.jsx.text.bundle.controls.f884667c"} defaultMessage={"Bundle Controls"} />
                        </h2>

                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
                            <StableText id={"common.jsx.text.theme.fe958fe1"} defaultMessage={"Theme"} />
                        </label>
                        <PremiumSelect
                            value={theme}
                        ...
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mt-4 mb-2">
                            <StableText id={"common.jsx.text.budget.c9a30fc0"} defaultMessage={"Budget ("} />{formatBasePrice(formatPrice, budget)})
                        </label>
                        <input
                            type="range"
                            min={5000}
                        ...
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mt-4 mb-2">
                            <StableText id={"common.jsx.text.max.items.ca714e15"} defaultMessage={"Max Items"} />
                        </label>
                        <input
                            type="number"
                            min={2}
                        ...
                        >
                            ...
                            {loading ? <StableText id={"common.jsx.expression.building.8b01fe84"} defaultMessage={"Building..."} /> : <StableText id={"common.jsx.expression.rebuild.bundle.dc534506"} defaultMessage={"Rebuild Bundle"} />}
                        </button>
                    </aside>
                            ...
                            <div>
                                <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-400">
                                            {bundle.items?.length || 0} <StableText id={"common.jsx.text.items.budget.use.2bbd9815"} defaultMessage={"items • Budget use"} /> {bundle.budgetUtilization || 0}%
                                        </p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-sm text-slate-400"><StableText id={"common.jsx.text.bundle.total.f7950807"} defaultMessage={"Bundle Total"} /></p>
                                        <p className="text-2xl font-black text-cyan-100">{formatBasePrice(formatPrice, bundle.totalPrice || 0)}</p>
                                        <p className="text-xs text-emerald-300"><StableText id={"common.jsx.text.savings.ef1e285b"} defaultMessage={"Savings"} /> {formatBasePrice(formatPrice, bundle.savings || 0)}</p>
                                    </div>
                                </div>
                                ...
                                <div className="mb-4 rounded-2xl border border-white/10 bg-zinc-950/45 p-4">
                                    <div className="flex items-center justify-between gap-3">
                                        <div>
                                            <p className="text-[11px] font-black uppercase tracking-[0.2em] text-cyan-200"><StableText id={"common.jsx.text.frontier.ai.layer.21e85bce"} defaultMessage={"Frontier AI Layer"} /></p>
                                            <h3 className="mt-1 text-lg font-black text-white"><StableText id={"common.jsx.text.bundle.intelligence.53ded863"} defaultMessage={"Bundle Intelligence"} /></h3>
                                        </div>
                                        <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-slate-300">
                                            {aiLoading ? <StableText id={"common.jsx.expression.thinking.fabb1016"} defaultMessage={"Thinking"} /> : (aiSummary?.provider || 'local')}
                                        </span>
                                    </div>
                                    ...
                                    {aiLoading && (
                                        <div className="mt-4 inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-300">
                                            ...
                                            <StableText id={"common.jsx.text.explaining.bundle.strategy.3189b0ee"} defaultMessage={"Explaining bundle strategy..."} />
                                        </div>
                                    )}
                                    ...
                                    {!aiLoading && !aiError && aiSummary && (
                                        <>
                                            ...
                                            {aiBundleAction?.path && (
                                                ...
                                                >
                                                    <StableText id={"common.jsx.text.open.suggested.bundle.view.02976b67"} defaultMessage={"Open Suggested Bundle View"} />
                                                </button>
                                            )}
                                        </>
                                    )}
                                ...
                                <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    ...
                                    >
                                        ...
                                        <StableText id={"cart.jsx.text.add.all.to.cart.ae4c6b49"} defaultMessage={"Add All To Cart"} />
                                    </button>
                                    <button
                                        type="button"
