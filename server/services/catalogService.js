...
const {
    safeString, safeLower, safeNumber, toInt, makeId,
...
const buildBaseProductFilter = async (options = {}) => {
    ...
    const pendingFilterPromise = buildBaseProductFilterFresh().then((nextValue) => {
        baseProductFilterCache.value = nextValue;
        baseProductFilterCache.cachedAt = Date.now();
        return nextValue;
    });

    baseProductFilterCache.promise = pendingFilterPromise;

    try {
        const nextValue = await pendingFilterPromise;
        return { ...nextValue };
    } finally {
