...
const appendEscrowPaymentEvent = async ({
    ...
    const hashPayload = (p) => crypto.createHash('md5').update(JSON.stringify(p)).digest('hex').slice(0, 8); // nosemgrep: nodejs-md5 — non-secret 8-char log correlation hash; accepted in config/security/pqc-allowlist.json (MD5_USAGE)
