...
const hashString = (value = '') => {
    const digest = crypto.createHash('sha1').update(String(value)).digest('hex'); // nosemgrep: security.semgrep.nodejs-sha1 - deterministic artwork bucketing hash (identity stability, not a security primitive); accepted in config/security/pqc-allowlist.json
