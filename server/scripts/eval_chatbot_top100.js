...
const addPromptFactory = () => {
    ...
    [
        ...
        'why is cart failing',
        'where is the chat controller',
        'explain the assistant v2 route',
        'what powers the intelligence service',
        'where are the chatbot files',
...
const renderMarkdownReport = ({ summary, results }) => {
    const lines = [];
    lines.push('# Chatbot Top 100 Eval', '', `Generated at: ${new Date().toISOString()}`, '', '## Summary', '');
...
const main = async () => {
    ...
    const jsonPath = path.resolve(RUN_LOG_DIR, 'chatbot-top-100-eval.json');
    const mdPath = path.resolve(RUN_LOG_DIR, 'chatbot-top-100-eval.md');
