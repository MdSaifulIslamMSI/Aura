...
const REQUIRED_UNIQUE_INDEXES = [
    ...
    ['paymentevents', { eventId: 1 }, 'payment webhook event identity'],
    ['paymentoutboxtasks', { taskType: 1, intentId: 1 }, 'payment outbox capture dedupe'],
    ['paymentmethods', { user: 1, providerMethodId: 1 }, 'payment method identity'],
    ['idempotencyrecords', { key: 1, user: 1, route: 1 }, 'request idempotency identity'],
    ['orders', { user: 1, idempotencyKey: 1 }, 'order creation idempotency', { idempotencyKey: { $gt: '' } }],
    ['orderemailnotifications', { notificationId: 1 }, 'order email notification identity'],
    ['orderemailnotifications', { dedupeKey: 1 }, 'order email delivery dedupe'],
