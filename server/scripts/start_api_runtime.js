...
const { loadLocalEnvFiles, primeAwsParameterStoreEnv, primeRuntimeSecretsEnv, getRuntimeSecretBootstrapState } = require('../config/runtimeConfig');

const targetScript = path.resolve(__dirname, '..', 'index.js');
...
const bootstrap = async () => {
    loadLocalEnvFiles();
    if (typeof primeRuntimeSecretsEnv === 'function') {
        await primeRuntimeSecretsEnv({ logger: console });
    } else {
    ...
    child.on('exit', (code, signal) => {
        if (signal) {
            console.warn(`[runtime] api child exited via ${signal} after ${bootstrapState.source}`);
...
bootstrap().catch((error) => {
    console.error('[runtime] api bootstrap failed:', error?.stack || error?.message || error);
    process.exit(1);
});
